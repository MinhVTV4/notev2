...
const DEFAULT_GOAL_STATUS = 'inprogress'; // Trạng thái mục tiêu mặc định
...
// =====================================================================
//  DOM References (Giữ nguyên, không thay đổi)
// =====================================================================
...
// =====================================================================
//  Theme & Appearance Management (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Toàn bộ code quản lý theme giữ nguyên) ...
...
// =====================================================================
//  Notebook Data Management (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Toàn bộ code quản lý sổ tay giữ nguyên) ...
...
// =====================================================================
//  Note Data Management (CẬP NHẬT)
// =====================================================================

// ** CẬP NHẬT: saveNotes **
// Thêm các trường dueDate và goalStatus vào đối tượng note khi lưu
const saveNotes = () => {
    try {
        const notesToSave = notes.map(note => ({
            ...
            // --- THÊM MỚI ---
            dueDate: note.dueDate || null, // Lưu dueDate
            goalStatus: note.goalStatus || DEFAULT_GOAL_STATUS // Lưu goalStatus
            // --- KẾT THÚC THÊM MỚI ---
        }));
        ...
        if (e.name === 'QuotaExceededError') {
            alert("Lỗi: Dung lượng lưu trữ cục bộ đã đầy. Không thể lưu ghi chú.");
        } else {
...
// ** CẬP NHẬT: loadNotes **
// Khi tải ghi chú, đảm bảo các trường mới tồn tại với giá trị mặc định
const loadNotes = () => {
    ...
    if (storedNotes) {
        try {
            notes = JSON.parse(storedNotes).map(note => ({
                ...
                deletedTimestamp: note.deletedTimestamp || null,
                notebookId: note.notebookId || null,
                // --- THÊM MỚI: Gán giá trị mặc định nếu chưa có ---
                dueDate: note.dueDate || null, // Mặc định là null nếu chưa có
                goalStatus: note.goalStatus || DEFAULT_GOAL_STATUS // Mặc định là 'inprogress' nếu chưa có
                // --- KẾT THÚC THÊM MỚI ---
            }));
        } catch (e) {
            console.error("Lỗi đọc dữ liệu ghi chú từ localStorage:", e);
            alert("Lỗi khi đọc dữ liệu ghi chú đã lưu. Dữ liệu có thể bị lỗi. Sử dụng dữ liệu mặc định.");
            notes = [];
        }
    } else {
        // Xử lý chuyển đổi dữ liệu cũ (giữ nguyên)
        ...
        if (oldStoredNotes) {
            ...
            try {
                notes = JSON.parse(oldStoredNotes).map(note => ({
                    ...
                    deletedTimestamp: note.deletedTimestamp || null,
                    notebookId: null,
                    // --- THÊM MỚI: Thêm giá trị mặc định cho dữ liệu cũ ---
                    dueDate: null,
                    goalStatus: DEFAULT_GOAL_STATUS
                    // --- KẾT THÚC THÊM MỚI ---
                }));
                saveNotes(); // Lưu lại theo định dạng mới
...
// ** CẬP NHẬT: addNote **
// Khởi tạo giá trị mặc định cho các trường mục tiêu khi thêm note mới
const addNote = () => {
    const noteTitle = newNoteTitle.value.trim();
    const noteText = newNoteText.value;
    const tagString = newNoteTags.value;

    // TODO: Lấy giá trị dueDate và goalStatus từ các element UI mới (sẽ thêm sau)
    // Ví dụ: const dueDateValue = document.getElementById('new-note-due-date')?.value || null;
    // Ví dụ: const goalStatusValue = document.getElementById('new-note-status')?.value || DEFAULT_GOAL_STATUS;

    if (noteText.trim() || noteTitle) {
        const tags = parseTags(tagString);
        const now = Date.now();
        const assignedNotebookId = (currentNotebookId !== 'all' && !isViewingArchived && !isViewingTrash) ? parseInt(currentNotebookId) : null;
        ...
        const newNote = {
            ...
            // --- THÊM MỚI: Gán giá trị mặc định (hoặc lấy từ UI sau này) ---
            dueDate: null, // Tạm thời là null, sẽ lấy từ UI sau
            goalStatus: DEFAULT_GOAL_STATUS // Tạm thời là mặc định, sẽ lấy từ UI sau
            // --- KẾT THÚC THÊM MỚI ---
        };
...
// =====================================================================
//  Template Data Management (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Toàn bộ code quản lý template giữ nguyên) ...
...
// =====================================================================
//  Helper Functions & Event Handlers (CẬP NHẬT)
// =====================================================================
...
// ** CẬP NHẬT: updateNoteData **
// Thêm xử lý cho dueDate và goalStatus
const updateNoteData = (noteIndex, newData) => {
    if (noteIndex < 0 || noteIndex >= notes.length) return false;
    const note = notes[noteIndex];
    if (!note) return false;
    // Lấy các giá trị mới, bao gồm cả thuộc tính mục tiêu
    const { title, text, tags, color, notebookId, dueDate, goalStatus } = newData;
    let changed = false;
    ...
    // --- THÊM MỚI: Làm sạch dữ liệu mục tiêu ---
    // Đảm bảo dueDate là định dạng 'YYYY-MM-DD' hoặc null
    const cleanDueDate = (typeof dueDate === 'string' && dueDate.match(/^\d{4}-\d{2}-\d{2}$/)) ? dueDate : null;
    // Đảm bảo goalStatus là 'inprogress' hoặc 'completed'
    const cleanGoalStatus = (goalStatus === 'completed') ? 'completed' : DEFAULT_GOAL_STATUS;
    // --- KẾT THÚC THÊM MỚI ---

    // So sánh và cập nhật các trường cơ bản
    ...
    // --- THÊM MỚI: So sánh và cập nhật các trường mục tiêu ---
    if (note.dueDate !== cleanDueDate) { note.dueDate = cleanDueDate; changed = true; }
    if (note.goalStatus !== cleanGoalStatus) { note.goalStatus = cleanGoalStatus; changed = true; }
    // --- KẾT THÚC THÊM MỚI ---

    if (changed) {
        note.lastModified = Date.now();
        saveNotes(); // Lưu lại nếu có thay đổi
        return true;
    }
    return false;
};

// ** CẬP NHẬT: debouncedAutoSave **
// Cần lấy giá trị dueDate và goalStatus từ UI (khi UI được cập nhật) để truyền vào updateNoteData
const debouncedAutoSave = debounce((noteElement, noteIndex) => {
    ...
    // TODO: Thêm selector cho các input dueDate và goalStatus mới trong chế độ sửa
    // const editDueDateInputCheck = noteElement.querySelector('input.edit-due-date-input');
    // const editGoalStatusInputCheck = noteElement.querySelector('input.edit-goal-status-input'); // Hoặc select/checkbox

    if (!editTitleInputCheck || !editInputCheck || !editTagsInputCheck || !noteElement.isConnected) {
        // Tạm thời chưa kiểm tra input mới
        return;
    }
    ...
    // --- THÊM MỚI: Lấy giá trị mục tiêu từ UI (khi có) ---
    let newDueDate = notes[noteIndex]?.dueDate; // Giữ giá trị cũ nếu chưa có input
    let newGoalStatus = notes[noteIndex]?.goalStatus; // Giữ giá trị cũ nếu chưa có input
    // if (editDueDateInputCheck) {
    //     newDueDate = editDueDateInputCheck.value || null;
    // }
    // if (editGoalStatusInputCheck) {
    //     // Logic lấy status từ checkbox/select...
    //     // newGoalStatus = editGoalStatusInputCheck.checked ? 'completed' : 'inprogress';
    // }
    // --- KẾT THÚC THÊM MỚI ---
    ...
    if (!wasPreviouslyEmpty && isNowEmpty) {
        // Tạm thời bỏ qua việc lưu nếu note trở nên trống rỗng (có thể cần logic khác)
        return;
    }

    // Truyền các giá trị mới vào updateNoteData
    const saved = updateNoteData(noteIndex, {
        title: newTitle,
        text: newText,
        tags: newTags,
        color: newColor,
        dueDate: newDueDate,         // Truyền dueDate
        goalStatus: newGoalStatus    // Truyền goalStatus
    });
    ...
    if (saved) {
        ...
        // Cập nhật lại timestamp hiển thị nếu cần
        const timestampEl = noteElement.querySelector('.note-timestamp');
        if(timestampEl && notes[noteIndex]) {
             const updatedTimestampEl = createNoteTimestampElement(notes[noteIndex]);
             if(updatedTimestampEl) timestampEl.textContent = updatedTimestampEl.textContent;
        }
    }
}, DEBOUNCE_DELAY);


// ** CẬP NHẬT: handleNoteEdit **
// Cần thêm các input cho dueDate và goalStatus vào chế độ sửa
const handleNoteEdit = (noteElement, noteId, noteIndex) => {
    ...
    // Lưu lại HTML gốc của các nút action
    ...
    // --- THÊM MỚI: Tạo input cho Mục tiêu ---
    const goalEditContainer = document.createElement('div');
    goalEditContainer.classList.add('goal-edit-container', 'mt-3', 'pt-3', 'border-t', 'border-dashed'); // Thêm style nếu cần

    // Input Due Date
    const dueDateLabel = document.createElement('label');
    dueDateLabel.textContent = 'Hạn chót:';
    dueDateLabel.htmlFor = `edit-due-date-${noteId}`;
    dueDateLabel.classList.add('block', 'text-sm', 'font-medium', 'text-gray-600', 'mb-1'); // Style label
    const editDueDateInput = document.createElement('input');
    editDueDateInput.type = 'date';
    editDueDateInput.id = `edit-due-date-${noteId}`;
    editDueDateInput.classList.add('edit-due-date-input', 'w-full', 'p-1', 'border', 'rounded-md', 'text-sm'); // Style input date
    editDueDateInput.value = noteData.dueDate || ''; // Gán giá trị hiện tại
    goalEditContainer.appendChild(dueDateLabel);
    goalEditContainer.appendChild(editDueDateInput);

    // Input Goal Status (dùng checkbox ví dụ)
    const statusLabel = document.createElement('label');
    statusLabel.htmlFor = `edit-goal-status-${noteId}`;
    statusLabel.classList.add('flex', 'items-center', 'mt-2', 'text-sm', 'cursor-pointer'); // Style label checkbox
    const editGoalStatusInput = document.createElement('input');
    editGoalStatusInput.type = 'checkbox';
    editGoalStatusInput.id = `edit-goal-status-${noteId}`;
    editGoalStatusInput.classList.add('edit-goal-status-input', 'mr-2', 'h-4', 'w-4', 'rounded'); // Style checkbox
    editGoalStatusInput.checked = noteData.goalStatus === 'completed'; // Check nếu đã hoàn thành
    statusLabel.appendChild(editGoalStatusInput);
    statusLabel.appendChild(document.createTextNode('Đã hoàn thành'));
    goalEditContainer.appendChild(statusLabel);
    // --- KẾT THÚC THÊM MỚI ---


    // Tạo bộ chọn màu (giữ nguyên)
    ...
    NOTE_COLORS.forEach(color => {
        ...
        swatchBtn.addEventListener('click', () => {
            ...
            debouncedAutoSave(noteElement, noteIndex); // Trigger autosave khi đổi màu
        });
    ...
    saveBtn.classList.add('save-edit-btn', 'modal-button', 'primary'); // Dùng class modal-button cho đồng bộ
    saveBtn.textContent = 'Lưu';
    saveBtn.title = 'Lưu thay đổi (Ctrl+S)';
    // ** CẬP NHẬT: onclick cho nút Lưu để lấy cả dữ liệu mục tiêu **
    saveBtn.onclick = () => handleNoteSaveEdit(noteElement, noteId, noteIndex);


    // Xóa nội dung cũ và thêm các element mới vào noteElement
    const bookmarkIcon = noteElement.querySelector('.pinned-bookmark-icon'); // Giữ lại icon ghim nếu có
    noteElement.innerHTML = ''; // Xóa sạch nội dung cũ
    if (bookmarkIcon) {
        noteElement.appendChild(bookmarkIcon);
        bookmarkIcon.style.display = 'inline-block'; // Đảm bảo icon hiển thị
    }
    noteElement.appendChild(editTitleInput);
    noteElement.appendChild(editInput);
    noteElement.appendChild(editTagsInput);
    noteElement.appendChild(goalEditContainer); // Thêm container input mục tiêu
    noteElement.appendChild(colorSelectorContainer); // Thêm bộ chọn màu

    // Tạo container mới cho các nút action (bao gồm nút Lưu)
    ...
    editActionsContainer.appendChild(saveBtn); // Thêm nút Lưu mới
    noteElement.appendChild(editActionsContainer);

    // Gắn listener cho autosave
    ...
    editTagsInput.addEventListener('input', (event) => { handleTagInput(event); triggerAutoSave(); });
    editTagsInput.addEventListener('blur', handleTagInputBlur, true);
    editTagsInput.addEventListener('keydown', handleTagInputKeydown);
    // --- THÊM MỚI: Gắn listener cho input mục tiêu ---
    editDueDateInput.addEventListener('input', triggerAutoSave);
    editGoalStatusInput.addEventListener('change', triggerAutoSave); // Dùng 'change' cho checkbox
    // --- KẾT THÚC THÊM MỚI ---


    editTitleInput.focus(); // Focus vào tiêu đề khi bắt đầu sửa
    editTitleInput.setSelectionRange(editTitleInput.value.length, editTitleInput.value.length); // Đưa con trỏ về cuối
};


// ** CẬP NHẬT: handleNoteSaveEdit **
// Lấy giá trị dueDate và goalStatus từ form sửa và truyền vào updateNoteData
const handleNoteSaveEdit = (noteElement, noteId, noteIndex) => {
    ...
    // --- THÊM MỚI: Lấy element input mục tiêu ---
    const editDueDateInput = noteElement.querySelector('input.edit-due-date-input');
    const editGoalStatusInput = noteElement.querySelector('input.edit-goal-status-input');
    // --- KẾT THÚC THÊM MỚI ---


    if (!editTitleInput || !editInput || !editTagsInput || !editDueDateInput || !editGoalStatusInput) {
        console.error("Lỗi lưu: Không tìm thấy các thành phần sửa ghi chú hoặc mục tiêu.");
        displayNotes(searchInput.value); // Render lại để thoát chế độ sửa lỗi
        return;
    }
    ...
    const selectedColorValue = noteElement.dataset.selectedColor ?? notes[noteIndex]?.color;
    const newColor = selectedColorValue;
    // --- THÊM MỚI: Lấy giá trị mục tiêu ---
    const newDueDate = editDueDateInput.value || null; // Lấy giá trị date hoặc null nếu trống
    const newGoalStatus = editGoalStatusInput.checked ? 'completed' : 'inprogress'; // Lấy từ checkbox
    // --- KẾT THÚC THÊM MỚI ---

    // Kiểm tra note trống (giữ nguyên)
    ...
    // Gọi hàm cập nhật dữ liệu, truyền cả thông tin mục tiêu
    ...
    // Lấy lại dữ liệu note đã cập nhật (quan trọng)
    const updatedNoteData = notes[noteIndex];

    // Render lại note ở chế độ xem (giữ nguyên logic render cơ bản)
    const bookmarkIcon = noteElement.querySelector('.pinned-bookmark-icon');
    noteElement.innerHTML = ''; // Xóa nội dung chế độ sửa
    if (bookmarkIcon) noteElement.appendChild(bookmarkIcon); // Thêm lại icon ghim nếu có

    applyNoteColor(noteElement, updatedNoteData);
    applyPinnedStatus(noteElement, updatedNoteData, isViewingArchived, isViewingTrash);

    // Tạo lại các thành phần hiển thị (cần cập nhật các hàm này sau để hiển thị dueDate, goalStatus)
    ...
    // TODO: Thêm phần hiển thị DueDate và GoalStatus vào đây hoặc trong hàm renderNoteElement
    // Ví dụ:
    // if (updatedNoteData.dueDate) {
    //     const dueDateDisplay = document.createElement('p');
    //     dueDateDisplay.classList.add('note-due-date'); // Thêm class CSS
    //     // Tính toán ngày còn lại và hiển thị
    //     dueDateDisplay.textContent = `Hạn: ${formatDisplayDate(updatedNoteData.dueDate)} (...)`;
    //     noteElement.appendChild(dueDateDisplay);
    // }
    // if (updatedNoteData.goalStatus === 'completed') {
    //     // Thêm dấu hiệu hoàn thành
    //     noteElement.classList.add('goal-completed'); // Thêm class CSS
    // }
    ...
    const actionsEl = createNoteActionsElement(updatedNoteData); // Tạo lại các nút action
    if(actionsEl) noteElement.appendChild(actionsEl);

    // Dọn dẹp
    delete noteElement.dataset.selectedColor; // Xóa màu đang chọn tạm
    hideTagSuggestions();
...
// =====================================================================
//  Core Note Rendering Function (CẦN CẬP NHẬT SAU)
// =====================================================================
// Hàm này cần được cập nhật để gọi các hàm helper mới (hoặc sửa hàm cũ)
// nhằm hiển thị thông tin dueDate và goalStatus.
const renderNoteElement = (note) => {
    ...
    // Thêm class nếu là mục tiêu đã hoàn thành (ví dụ)
    if (note.goalStatus === 'completed') {
        noteElement.classList.add('goal-completed'); // Cần định nghĩa class này trong CSS
    }
    ...
    // --- TODO: Thêm hiển thị DueDate và Goal Status ---
    // Ví dụ:
    // if (note.dueDate) {
    //     const dueDateDisplay = document.createElement('p');
    //     dueDateDisplay.classList.add('note-due-date'); // Thêm class CSS
    //     // ... tính ngày còn lại ...
    //     dueDateDisplay.textContent = `Hạn: ${formatDisplayDate(note.dueDate)} (...)`;
    //     noteElement.appendChild(dueDateDisplay);
    // }
    // --- KẾT THÚC TODO ---
...
// =====================================================================
//  Drag & Drop (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Code kéo thả giữ nguyên) ...
...
// =====================================================================
//  Tag Handling (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Code xử lý tag giữ nguyên) ...
...
// =====================================================================
//  Template UI Handlers (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Code xử lý UI template giữ nguyên) ...
...
// =====================================================================
//  Notebook UI Handlers (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Code xử lý UI notebook giữ nguyên) ...
...
// =====================================================================
//  Notebook Tab Rendering (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Code render tab notebook giữ nguyên) ...
...
// =====================================================================
//  Other Panel/Import/Export (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Code xử lý panel thêm, import/export giữ nguyên) ...
...
const sortNotes = (filteredNotes) => { if (isViewingTrash) { return filteredNotes.sort((a, b) => (b.deletedTimestamp || b.lastModified) - (a.deletedTimestamp || a.lastModified)); } else if (isViewingArchived) { return filteredNotes.sort((a, b) => (b.lastModified || b.id) - (a.lastModified || a.id)); } else { return filteredNotes.sort((a, b) => { if (currentNotebookId === 'all' && a.pinned !== b.pinned) { return b.pinned - a.pinned; } // TODO: Thêm logic sắp xếp theo dueDate, goalStatus nếu cần return (b.lastModified || b.id) - (a.lastModified || a.id); }); } };

// =====================================================================
//  Core Display Function (CẬP NHẬT)
// =====================================================================
// Hàm này gọi renderNoteElement, nên khi renderNoteElement được cập nhật,
// hiển thị sẽ tự động thay đổi.
const displayNotes = (filter = '') => {
    ...
    const notesToDisplay = sortNotes(filteredNotes); // Sắp xếp

    // Cập nhật trạng thái UI (giữ nguyên)
    ...
    if (notesToDisplay.length === 0) {
        ...
        if (sortableInstance) { sortableInstance.destroy(); sortableInstance = null; } // Hủy kéo thả nếu không có note
    } else {
        notesToDisplay.forEach(note => {
            const noteElement = renderNoteElement(note); // Gọi hàm render đã cập nhật (sẽ cập nhật sau)
            notesContainer.appendChild(noteElement);
        });
        initSortable(); // Khởi tạo lại kéo thả
    }

    window.scrollTo({ top: scrollY, behavior: 'instant' }); // Giữ vị trí cuộn
};


// =====================================================================
//  Modal Handling Functions (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Code xử lý modal settings, move note giữ nguyên) ...
...
// =====================================================================
//  Event Listener Setup Functions (Giữ nguyên, không thay đổi)
// =====================================================================
// ... (Toàn bộ code setup event listener giữ nguyên) ...
...
const setupNoteActionListeners = () => { notesContainer.addEventListener('click', (event) => { const target = event.target; const noteElement = target.closest('.note'); if (!noteElement) return; const noteId = parseInt(noteElement.dataset.id); const noteIndex = notes.findIndex(note => note.id === noteId); if (noteIndex === -1) { console.error("Không tìm thấy data cho note ID:", noteId); return; } const tagButton = target.closest('.tag-badge'); if (tagButton?.dataset.tag) { event.preventDefault(); event.stopPropagation(); searchInput.value = `#${tagButton.dataset.tag}`; searchInput.dispatchEvent(new Event('input', { bubbles: true })); searchInput.focus(); window.scrollTo({ top: 0, behavior: 'smooth' }); return; } const readMoreButton = target.closest('.read-more-btn'); if (readMoreButton) { event.stopPropagation(); const note = notes[noteIndex]; if (note) showFullNoteModal(note.title, note.text); return; } const isEditingThisNote = noteElement.querySelector('textarea.edit-input'); if (isEditingThisNote) { if (target.closest('.save-edit-btn')) { handleNoteSaveEdit(noteElement, noteId, noteIndex); } else if (target.closest('.pin-btn') && currentNotebookId === 'all') { handleNotePin(noteId, noteIndex); const pinBtn = target.closest('.pin-btn'); if (pinBtn) { const isPinned = notes[noteIndex].pinned; pinBtn.title = isPinned ? "Bỏ ghim" : "Ghim ghi chú"; pinBtn.setAttribute('aria-label', isPinned ? "Bỏ ghim ghi chú" : "Ghim ghi chú"); pinBtn.setAttribute('aria-pressed', isPinned ? 'true' : 'false'); pinBtn.classList.toggle('pinned', isPinned); } } return; } const moveButton = target.closest('.move-note-btn'); if (moveButton && !isViewingArchived && !isViewingTrash) { event.stopPropagation(); showMoveNoteMenu(noteId, moveButton); return; } if (target.closest('.pin-btn') && !isViewingArchived && !isViewingTrash && currentNotebookId === 'all') handleNotePin(noteId, noteIndex); else if (target.closest('.delete-btn')) handleNoteDelete(noteId, noteIndex); else if (target.closest('.archive-btn') && !isViewingTrash && !isViewingArchived) handleNoteArchive(noteId, noteIndex); else if (target.closest('.unarchive-btn') && isViewingArchived) handleNoteUnarchive(noteId, noteIndex); else if (target.closest('.restore-btn') && isViewingTrash) handleNoteRestore(noteId, noteIndex); else if (target.closest('.delete-permanent-btn') && isViewingTrash) handleNoteDeletePermanent(noteId, noteIndex); else if (target.closest('.edit-btn') && !isViewingArchived && !isViewingTrash) handleNoteEdit(noteElement, noteId, noteIndex); }); };
...
const setupGlobalListeners = () => { document.addEventListener('mousedown', (event) => { if (activeMoveMenu && !activeMoveMenu.contains(event.target) && !event.target.closest('.move-note-btn')) { closeMoveNoteMenu(); } const suggestionBox = document.getElementById(SUGGESTION_BOX_ID); if (suggestionBox && !suggestionBox.contains(event.target) && activeTagInputElement && !activeTagInputElement.contains(event.target)) { hideTagSuggestions(); } }, true); setupGlobalKeydownListeners(); };
const setupGlobalKeydownListeners = () => { document.addEventListener('keydown', (event) => { const activeElement = document.activeElement; const isTyping = activeElement && (activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA') && activeElement !== searchInput; const isTemplateModalOpen = templateModal.classList.contains('visible'); const isNoteModalOpen = !!document.querySelector('.note-modal.visible'); const isSettingsModalOpen = settingsModal.classList.contains('visible'); const isNotebookModalOpen = notebookModal.classList.contains('visible'); const isSuggestionBoxOpen = !!document.getElementById(SUGGESTION_BOX_ID); const isMoveMenuOpen = !!activeMoveMenu; const isEditingNote = activeElement?.closest('.note')?.querySelector('.edit-input, .edit-title-input, .edit-tags-input, .edit-due-date-input, .edit-goal-status-input') === activeElement; const isEditingTemplate = templateEditPanel.contains(activeElement); const isEditingNotebook = notebookEditPanel.contains(activeElement); if (event.key === 'Escape') { if (isMoveMenuOpen) closeMoveNoteMenu(); else if (isSuggestionBoxOpen) hideTagSuggestions(); else if (isSettingsModalOpen) hideSettingsModal(); else if (isNoteModalOpen) document.querySelector('.note-modal.visible .close-modal-btn')?.click(); else if (isTemplateModalOpen) { if (!templateEditPanel.classList.contains('hidden')) hideTemplateEditPanel(); else hideTemplateModal(); } else if (isNotebookModalOpen) { if (!notebookEditPanel.classList.contains('hidden')) hideNotebookEditPanel(); else hideNotebookModal(); } else if (!addNotePanel.classList.contains('hidden')) hideAddPanel(); else if (isEditingNote) { const editingNoteElement = activeElement.closest('.note'); if (editingNoteElement && confirm("Bạn có muốn hủy bỏ các thay đổi và đóng chỉnh sửa ghi chú không?")) { displayNotes(searchInput.value); if (addNotePanel.classList.contains('hidden')) showAddPanelBtn.classList.remove('hidden'); if (sortableInstance) sortableInstance.option('disabled', false); } } else if (activeElement === searchInput && searchInput.value !== '') { searchInput.value = ''; displayNotes(); } event.preventDefault(); event.stopPropagation(); return; } const isAnyModalOpen = isNoteModalOpen || isTemplateModalOpen || isSettingsModalOpen || isNotebookModalOpen; const allowSaveInModal = (event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 's' && (isEditingTemplate || isEditingNotebook); if ((isAnyModalOpen && !allowSaveInModal) || isMoveMenuOpen) return; if (isTyping && !isEditingNote && !isEditingTemplate && !isEditingNotebook) return; const isCtrlOrCmd = event.metaKey || event.ctrlKey; if (isCtrlOrCmd && event.key.toLowerCase() === 'n') { event.preventDefault(); if (!isAnyModalOpen && addNotePanel.classList.contains('hidden') && !notesContainer.querySelector('.note .edit-input')) { showAddPanel(); } } else if (isCtrlOrCmd && event.key.toLowerCase() === 's') { if (isEditingNote) { event.preventDefault(); activeElement.closest('.note')?.querySelector('.save-edit-btn')?.click(); } else if (addNotePanel.contains(activeElement)) { event.preventDefault(); addNoteBtn.click(); } else if (isEditingTemplate) { event.preventDefault(); saveTemplateBtn.click(); } else if (isEditingNotebook) { event.preventDefault(); saveNotebookBtn.click(); } } else if (isCtrlOrCmd && event.key.toLowerCase() === 'f') { event.preventDefault(); searchInput.focus(); searchInput.select(); } }); };
const setupEventListeners = () => { setupThemeAndAppearanceListeners(); setupHeaderActionListeners(); setupAddNotePanelListeners(); setupSearchListener(); setupNoteActionListeners(); setupTemplateModalListeners(); setupNotebookListeners(); setupTagInputListeners(); setupGlobalListeners(); };


// =====================================================================
//  Initial Load Function (Giữ nguyên, không thay đổi)
// =====================================================================
const loadNotesAndInit = () => {
     loadNotes(); // Đã được cập nhật để xử lý trường mới
     loadTemplates();
     loadNotebooks();
     ...
     renderNotebookTabs();
     displayNotes(); // Gọi hàm display đã cập nhật
     populateTemplateDropdown();
     setupEventListeners();
};

// =====================================================================
//  Start the application (Giữ nguyên, không thay đổi)
// =====================================================================
loadNotesAndInit();
```
...
* **`loadNotes`:** Đảm bảo các ghi chú cũ hoặc ghi chú được import thiếu `dueDate` hoặc `goalStatus` sẽ được gán giá trị mặc định (`null` và `'inprogress'`).
* **`saveNotes`:** Bao gồm `dueDate` và `goalStatus` khi lưu ghi chú vào `localStorage`.
* **`addNote`:** Khởi tạo `dueDate` và `goalStatus` với giá trị mặc định cho ghi chú mới (sẽ lấy từ UI sau).
* **`updateNoteData`:** Thêm logic để làm sạch và cập nhật `dueDate` và `goalStatus` khi có dữ liệu mới được truyền vào.
* **`handleNoteEdit`:** Thêm các phần tử `<input type="date">` và `<input type="checkbox">` vào chế độ sửa để người dùng có thể nhập/thay đổi hạn chót và trạng thái hoàn thành. Gắn listener cho các input này để kích hoạt `debouncedAutoSave`.
* **`handleNoteSaveEdit`:** Lấy giá trị từ các input `dueDate` và `goalStatus` mới khi người dùng nhấn nút lưu và truyền chúng vào hàm `updateNoteData`.
* **`debouncedAutoSave`:** Cập nhật để lấy giá trị `dueDate` và `goalStatus` từ UI (khi UI được cập nhật) và truyền vào `updateNoteData`.

**Bước tiếp theo:**

1.  **Cập nhật Giao diện Thêm/Sửa:**
    * Thêm các ô nhập liệu (`<input type="date">`, checkbox/select cho trạng thái) vào `#add-note-panel` trong file `index.html`.
    * Trong `script.js`, cập nhật hàm `addNote` để lấy giá trị từ các ô nhập liệu mới này.
    * Cập nhật hàm `handleNoteEdit` (đã làm một phần ở trên) và `handleNoteSaveEdit` để xử lý các input này trong chế độ sửa inline.
2.  **Cập nhật Giao diện Hiển thị:**
    * Trong `script.js`, cập nhật hàm `renderNoteElement` (và các hàm helper liên quan như `createNoteTitleElement`, `createNoteContentElement` nếu cần) để hiển thị thông tin `dueDate` (định dạng ngày, tính ngày còn lại) và `goalStatus` (icon, gạch ngang, đổi màu...) trên thẻ ghi chú ở chế độ xem.
    * Thêm các lớp CSS cần thiết vào `style.css` để định dạng hiển thị này (ví dụ: `.note-due-date`, `.goal-completed`).

Bạn muốn chúng ta tiếp tục với việc **cập nhật giao diện Thêm/Sửa** hay **cập nhật giao diện Hiển thị** trư
